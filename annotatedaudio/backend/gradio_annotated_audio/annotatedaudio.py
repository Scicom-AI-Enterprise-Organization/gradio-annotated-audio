...
class AnnotatedAudio(
    ...
    def __init__(
        ...
        *,
        ...
        """
        Parameters:
            value: A path, URL, or [sample_rate, numpy array] tuple (sample rate in Hz, audio data as a float or int numpy array) for the default value that AnnotatedAudio component is going to take. If a function is provided, the function will be called each time the app loads to set the initial value of this component.
        ...
        self.categories = (
            self._process_categories(categories) if categories is not None else None
        )
        category_keys = (
            {c["key"] for c in self.categories} if self.categories else set()
        )
        self.annotations = (
            self._process_annotations(annotations, category_keys)
            if annotations is not None
            else None
        )
        self.transcript = (
            self._process_transcript(transcript) if transcript is not None else None
        )
    ...
    def _process_categories(
        ...
        for i, category in enumerate(categories):
            ...
            if "key" not in category:
                raise ValueError(f"Category at index {i} missing required 'key' field")
            key = category["key"]
            if key in seen_keys:
    ...
    def _process_annotations(
        annotations: list[dict[str, Any]], category_keys: set[str]
    ) -> list[dict[str, Any]]:
        processed = []
        for i, annotation in enumerate(annotations):
            if not isinstance(annotation, dict):
                raise ValueError(f"Annotation at index {i} must be a dictionary")
            if "category" not in annotation:
                raise ValueError(
                    f"Annotation at index {i} missing required 'category' field"
                )
            category = annotation["category"]
            if category_keys and category not in category_keys:
                raise ValueError(
                    f"Annotation at index {i} references unknown category {category!r}"
                )
            kind = annotation.get("kind")
            if kind not in ("point", "range"):
                raise ValueError(
                    f"Annotation at index {i} 'kind' must be 'point' or 'range'"
                )
            entry: dict[str, Any] = {
                "category": category,
                "kind": kind,
            ...
            if kind == "point":
                if "time" not in annotation:
                    raise ValueError(
                        f"Point annotation at index {i} missing required 'time' field"
                    )
                entry["time"] = annotation["time"]
            else:
                if "start" not in annotation or "end" not in annotation:
                    raise ValueError(
                        f"Range annotation at index {i} missing required 'start'/'end' fields"
                    )
                entry["start"] = annotation["start"]
                entry["end"] = annotation["end"]
            processed.append(entry)
        return processed

    @staticmethod
    def _process_transcript(transcript: list[dict[str, Any]]) -> list[dict[str, Any]]:
        processed = []
        for i, segment in enumerate(transcript):
            if not isinstance(segment, dict):
                raise ValueError(f"Transcript segment at index {i} must be a dictionary")
            for field in ("start", "end", "text"):
                if field not in segment:
                    raise ValueError(
                        f"Transcript segment at index {i} missing required {field!r} field"
                    )
            processed.append(
                {
                    "start": segment["start"],
    ...
    async def covert_to_adts(data: bytes) -> tuple[bytes, float]:
        return await anyio.to_thread.run_sync(AnnotatedAudio._convert_to_adts, data)
