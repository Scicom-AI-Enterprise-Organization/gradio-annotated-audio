<script lang="ts">
	...
	import AnnotationLanes from "../shared/AnnotationLanes.svelte";
	import TranscriptPanel from "../shared/TranscriptPanel.svelte";
	...
	function seek_and_play(time: number): void {
		if (!waveform) return;
		waveform.play(time);
	}
...
{:else if use_waveform}
	...
	>
		...
		<TranscriptPanel {transcript} {current_time} onseek={seek_and_play} />
	</div>
{/if}
